...
class HttpCommentRepository implements CommentRepository {
    async CreateComment(jwt: string, idea_id: number, content: string, priority_emoji: string): Promise<AxiosResponse | undefined> {
        try {
            return await httpClient.post('/comments', { idea_id, content, priority_emoji },
                                                    { 
