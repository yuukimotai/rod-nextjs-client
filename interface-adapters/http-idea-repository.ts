...
class HttpIdeaRepository implements IdeaRepository {
    ...
    async ShowSharedIdeas(jwt: string): Promise<AxiosResponse | undefined> {
        try {
            return await httpClient.get('/shared/ideas', {headers: {"Content-Type": "application/json", "Authorization": `Bearer ${jwt}`}});
        } catch(error) {
            console.error(`${error}: サーバーに接続できませんでした`);
        }
    }
