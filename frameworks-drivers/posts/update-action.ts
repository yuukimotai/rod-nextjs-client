...
const UpdateAction = async (postId: number, title: string, content: string): Promise<{status: number, title: string}> => {
    ...
    if (jwt) {
        result = await updatePostUseCase.execute(jwt.value, postId, title, content, "");
    }
